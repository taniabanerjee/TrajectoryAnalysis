        ...
        while (reportTime < currentTime):
            ...
            if ('-' not in sigstatus.decode()):
                ...
                phasestr = '0' * paddedzeroes + hexstr
                    ...
                    #debug = 1
                spatrow = (options['cityintid'], reportTime.strftime("%Y-%m-%d %H:%M:%S.%f"), phasestr, str(cycle_no), 'Gainesville','FL')
...
def processTrack(tid, df, spatdf, geoclusters, centroid_list, centroids, tH):#, offset, mycursor, myoutputdb):
    updaterows = []
    insertrows = []
    inserttrackrows = []
    updateindex = -1
    ...
    if (x.size == 0):
        ...
        nt.ts = ts
        nt.f = f
        return updaterows, insertrows, inserttrackrows, nt
    phaseindex = ts.size - 2
    if (phaseindex < 0):
        phaseindex = ts.size - 1
    exactmatch=spatdf[spatdf['timestamp']==ts[phaseindex]+f[phaseindex]*np.timedelta64(100,'ms')]
    if (exactmatch.empty == False):
        rs = exactmatch['hexphase'].values[0]
        cycle = exactmatch['cycle'].values[0]
    else:
        temp = ts[phaseindex]+f[phaseindex]*np.timedelta64(100,'ms')
        while (exactmatch.empty):
            exactmatch=spatdf[spatdf['timestamp']==(temp - np.timedelta64(100,'ms'))]
            temp = temp - np.timedelta64(100,'ms')
        if (exactmatch.empty == False):
            rs = exactmatch['hexphase'].values[0]
            cycle = exactmatch['cycle'].values[0]
    nt = tH[tid]
    ...
    if (spindex > -1):
        exactmatch=spatdf[spatdf['timestamp']==ts[spindex]+f[spindex]*np.timedelta64(100,'ms')]
        if (exactmatch.empty == False):
            speedphase = exactmatch['hexphase'].values[0]
        else:
            temp = ts[spindex]+f[spindex]*np.timedelta64(100,'ms')
            while (exactmatch.empty):
                exactmatch=spatdf[spatdf['timestamp']==(temp - np.timedelta64(100,'ms'))]
                temp = temp - np.timedelta64(100,'ms')
            if (exactmatch.empty == False):
    ...
    if (updateindex > -1):
        si = updateindex
        if (oldclus != nt.clus):
            updaterow = (str(nt.phase), nt.clus, str(nt.isAnomalous), str(nt.isrj), str(tid))
    ...
    for index in range(si, ts.size-1):
        ...
        insertrow = (str(f[index]), str(tid), str(x[index]), str(y[index]), str(c[index-si]), tstr, str(iid[index-si]), rs, str(cycle), str(sx[index-si]), str(sy[index-si]))
        insertrows.append(insertrow)
    if (updateindex == -1):
        inserttrackrow = (str(nt.tid), str(nt.phase), nt.clus, str(nt.isAnomalous), str(nt.isrj), str(nm[0]), str(sig[0]))
...
def main():
    ...
    writequery='insert into OnlineDisplayInfo (frame_id, track_id, center_x, center_y, class, timestamp, intersection_id, SPAT, Cycle, speed_x, speed_y) values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);'
    writetrackquery='insert into TrackProperties (track_id, phase, cluster, isAnomalous, redJump, nearmiss, signature) values (%s, %s, %s, %s, %s, %s, %s);'
    updatetrackquery='update TrackProperties set phase=%s, cluster=%s, isAnomalous=%s, redJump=%s where track_id=%s;'
    ...
    phase2vehiclets = []
    offset = 0
    wait = datetime.timedelta(seconds=3600)
    clusterinterval = datetime.timedelta(seconds=3600)
    t2r = 0
    t2g = 0
    fftime = 0
    stopbar = literal_eval(options['phase2stopbar'])
    while (ftime < etime):
        results = []
        pool = mp.Pool(10)
        myimpl = time.time()
        ...
        pool.close()
        pool.join()
        #results.append(pool.apply_async(processTrack, args=(tid, df, spatdf, geoClusters, finalCentroidName, finalCentroids, tracksH)) for tid in tracks)
        #for tid in tracks:
            #updaterows, insertrows, inserttrackrows, nt = processTrack(tid, df, spatdf, geoClusters, finalCentroidName, finalCentroids, tracksH, offset, mycursor, myoutputdb)
            #tracksH[tid] = nt
        #pool.close()
        #pool.join()
        ...
        for i in range(0, len(tracks)):
            if (results[i].ready()):
                ...
                insertrows.extend(gotresults[1])
        ...
        print ("Finished processing 1 minute worth of records",end-start, mysqlwb-mysqlws, len(updaterows)+len(inserttrackrows), mysqlwe-mysqlwb, len(insertrows))
        print ("Finished processing 1 minute worth of records",myimpldone-myimpl, 'impl', len(tracks), (mypropcopy-myimpldone), 'pool', (mypropcopydone-mypropcopy), 'copy res')
        updaterows.clear()
        insertrows.clear()
        ...
        #wait = wait + step + minstep
            ...
            #for tid in tracks:
                #nt = tracksH[tid]
                #ts, righttrack = trackCrossesStopBar(nt, df, stopbar)
                #if (righttrack):
                    #phase2vehiclets.append(ts)
                    #if (t2r == 0):
                        #t2r = getlastphase2(stime, nt, spatdf, 0)
                    #if (t2g == 0 and t2r != 0):
                        #t2g = getphase2green(t2r, nt, spatdf, 1)
                        #t2c = getfirstvehphase2(t2g,t2r,phase2vehiclets)
                        #offset = t2c-t2g
...
#
        ...
        print ("Finished processing 1 minute worth of records", (mysqldfreaddone-mysqldfread), 'df read', (mysqlsdfread-mysqldfreaddone), 'unique tid')
...
def getlastphase2(ftime, nt, spatdf, greenbit):
    t2r = 0
    ts = nt.ts[0]
    fs = np.datetime64(ftime)
    while (ts > fs):
        exactmatch=spatdf[spatdf['timestamp']==ts]
        if (exactmatch.empty == False):
            rs = exactmatch['hexphase'].values[0]
            bin_string = bin(int('1'+rs, 16))[3:]
            if (bin_string[1] == greenbit):
                t2r = ts
                break;
...
def trackCrossesStopBar(nt, df, stopbar):
    righttrack = False
    ts = 0
    if (nt.gbit == 2):
        righttrack = True
