...
def getGreenBit(x, y):
    ...
    a = getCosineOfAngle(Ax, Ay, dir_6_x_26 - dir_2_x_26, dir_6_y_26 - dir_2_y_26)
    throughcos = float(options['through'])
    curvedcos = float(options['curved'])
    if (a > throughcos):
        bit = 2
    else:
        a = getCosineOfAngle(Ax, Ay, dir_2_x_62 - dir_6_x_62, dir_2_y_62 - dir_6_y_62)
        if (a > throughcos):
            bit = 6
        else:
            a = getCosineOfAngle(Ax, Ay, dir_8_x_48 - dir_4_x_48, dir_8_y_48 - dir_4_y_48)
            if (a > throughcos):
                bit = 4
            else:
                a = getCosineOfAngle(Ax, Ay, dir_4_x_84 - dir_8_x_84, dir_4_y_84 - dir_8_y_84)
                if (a > throughcos):
                    bit = 8
                else: # check curved paths
                    a = getCosineOfAngle(Ax, Ay, dir_4_x_48-dir_2_x_26, dir_4_y_48-dir_2_y_26)
                    if (a > curvedcos):
                        ...
                        if (dist < 50): #np.absolute needed to differentiate between left turn and the far right turn
                            bit = 5
                            bit2 = 2
                        else:
                            bit = 0
                        ...
                        a = getCosineOfAngle(Ax, Ay, dir_8_x_84-dir_6_x_62, dir_8_y_84-dir_6_y_62)
                        if (a > curvedcos):
                            ...
                            a = getCosineOfAngle(Ax, Ay, dir_6_x_62-dir_4_x_48, dir_6_y_62-dir_4_y_48)
                            if (a > curvedcos):
                                ...
                                a = getCosineOfAngle(Ax, Ay, dir_2_x_26-dir_8_x_84, dir_2_y_26-dir_8_y_84)
                                if (a > curvedcos):
...
def processTrack(tid, df, spatdf, geoclusters, centroid_list, centroids, tH):
    ...
    s4 = time.time()
    exactmatch = spatdf.iloc[getSPaTIndex(spatdf, ts[phaseindex]+(f[phaseindex]%10)*np.timedelta64(100,'ms'))]
    ...
    if (updateindex == -1):
        ...
        tstr = pdts.strftime("%Y-%m-%d %H:%M:%S")
        inserttrackrow = (options['cityintid'], str(iid[0]), tstr, str(nt.tid), str(nt.phase), nt.clus, str(nt.isAnomalous), str(nt.isrj), str(nm[0]), str(sig[0]))
        inserttrackrows.append(inserttrackrow)
        #mysqlwb = time.time()
        #mycursor.execute(writequery, insertrow)
        #myoutputdb.commit()
    s13 = time.time()
    #print (tid, ts.size, s2-s1, s3-s2, s4-s3, s5-s4, s6-s5, s7-s6, s8-s7, s9-s8, s10-s9, s11-s10, s12-s11, s13-s12)
    return updaterows, insertrows, inserttrackrows, nt
    ...
    while (ftime < etime):
        ...
        with mp.Pool(10) as pool:
            for tid in tracks:
                ...
                updaterows, insertrows, inserttrackrows, nt = processTrack(tid, df, spatdf, geoClusters, finalCentroidName, finalCentroids, tracksH)
                tracksH[tid] = nt
            prevstime = stime
            prevftime = ftime
            prevspatdf = spatdf
            ...
            df = pd.read_sql(querylimit, myinputdb, params={'name' : str(intersection_num), 'name2' : stime.strftime("%Y-%m-%d %H:%M:%S.%f"), 'name3' : ftime.strftime("%Y-%m-%d %H:%M:%S.%f")})
            #mdf = df.assign(NewTime=lambda x: x.timestamp + offset)
