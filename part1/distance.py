...
#@dataclass
class Track:
    def __init__(self, tid, xstart, xend, ystart, yend, rrange, detx):
...
def KMeansAssign(cdf, centroids, ptracks):
    s = len(centroids)
    for i in range (0,s):
        d = []
        nt = centroids[i]
...
def KMeansIterate(cdf, centroids, ptracks, rrange):
    j = 0
    while True:
        print ("iteration:", j)
        j = j+1
        ...
        update(centroids, cdf, rrange)
        cdf = KMeansAssign(cdf, centroids, ptracks)
        if closest_centroids.equals(cdf['closest']):
            break

def update(centroids, df, rrange):
    s = len(centroids)
    for i in range (0, s):
        ...
        for j in range(i+1, s-1):
            ...
            if (dist < 2000):
                ...
                if (clus1 == None and clus2 == None):
                    ...
                    if (closeToMembers(tid1, nhclus[tid2], df_, mclus, 2000)):
                        nhclus[tid1] = nhclus[tid2]
                        mclus[nhclus[tid1]].append(tid1)
                elif (clus2 == None):
                    if (closeToMembers(tid2, nhclus[tid1], df_, mclus, 2000)):
...
def printClustersToFile(cdf, filename, mode):
    df_tracks_with_clusters['track_id'] = cdf['track_id']
    df_tracks_with_clusters['closest'] = cdf['closest']
    df_tracks_with_clusters.to_csv(filename, mode=mode, header=False)
...
def KMeansInit(ptracks, rrange, idclus):
    #Calculate pairwise distance        
    #maximumdistance = 450000.0
    mclus = HashTable(50000)

    s = min(len(ptracks), 200)
    s = len(ptracks)
    #df_ stores pairwise similarity
        ...
        for j in range(i+1, s-1):
            ...
            if (dist < 2000):
                ...
                if (clus1 == None and clus2 == None):
                    ...
                    if (closeToMembers(tid1, hclus[tid2], df_, mclus, 2000)):
                    ...
                    if (closeToMembers(tid2, hclus[tid1], df_, mclus, 2000)):
...
def main():
    ...
    for tid in tracks:
        ...
        x = ndf.iloc[:,xindex].values
        y = ndf.iloc[:,yindex].values
        ...
        nt = Track(tid, x[0], x[r-1], y[0], y[r-1], rrange, detx)
        nt.arr = arr
        tracksH[tid] = nt
    ...
    idclus = 1
    idclus, centroids = KMeansInit(ptracks, 1280, idclus)
    cdf = pd.DataFrame()
    cdf['track_id'] = ptracks

    KMeansIterate(KMeansAssign(cdf, centroids, ptracks), centroids, ptracks, 1280)
    ...
    printClustersToFile(cdf, 'cluster.csv', 'w')
