...
class Track:
    def __init__(self, tid, xstart, xend, ystart, yend, rrange, detx):
        ...
        self.cid = -1
        self.classtype = 'car' #other types are bus, truck, ped
        self.phasetype = 'major' #major, minor, leftmajor, leftminor

df = pd.read_csv('outputclass.csv')
columns = ['track_id', 'x', 'y', 'class']
np.seterr(all='raise')
